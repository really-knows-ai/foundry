...
2. Ask the user how they want to finish:
   - **Squash merge** — call `foundry_git_finish` with a commit message and base branch. **Destructive**: deletes `WORK.md` / `WORK.history.yaml` / `WORK.feedback.yaml`, commits the cleanup on the work branch, checks out the base branch, squash-merges, commits with the user's message, and force-deletes the work branch. Requires `confirm: true` (without it returns a plan); refuses dirty worktrees; on merge conflict aborts and preserves the work branch.
