...
try {
  ...
  const client = {
    session: { create: async () => ({ id: 's1' }), prompt: async () => { console.log('  [prompt called]'); return { ok: true }; }, messages: async () => { console.log('  [messages called]'); return []; } },
    config: { providers: async () => { console.log('  [providers called]'); return []; } },
    provider: { list: () => { console.log('  [list called]'); return { connected: [] }; } },
  };
