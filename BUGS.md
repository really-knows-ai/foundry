...
- [x] **BUG-5: `foundry_workfile_create` stages don't use aliases.** The flow skill passed `["forge", "quench", "appraise"]` instead of `["forge:write-haiku", "quench:check-syllables", "appraise:evaluate-quality"]`. This feeds into BUG-3. Fixed together with BUG-3 via `enrichStages()`.
