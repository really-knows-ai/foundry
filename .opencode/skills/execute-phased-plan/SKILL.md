...
Requirements:
- Read `SPEC.md`, the plan, and current phase before editing.
...
After every phase is committed, run the repository's full quality gate. If the project defines `build:all`, use it. Otherwise run the best available lint, test, and build commands from the project documentation.

Fix failures through the same implementer-reviewer loop. Commit any final quality-gate fixes separately.
