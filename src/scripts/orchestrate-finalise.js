...
export function handleViolation(args) {
  ...
  if (!failedStage) { return violation('lastResult.ok=false but the orchestrator has no record of which stage failed — no active stage and no last stage on disk. Call foundry_run() without arguments to sort and get back on track'); }
  ...
  return violation(
    ...
    `Call foundry_run() to get the next action — the orchestrator will sort and route accordingly.`,
