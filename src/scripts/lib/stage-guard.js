...
export function requireNoActiveStage(io) {
  ...
  if (!a) return { ok: true };
  return { ok: false, error: `stage "${a.stage}" is already active — finish it with foundry_stage_end first. If abandoned, call foundry_run() to recover` };
}
...
export function requireActiveStage(io, { stageBase, cycle } = {}) {
  const a = readActiveStage(io);
  if (!a) return { ok: false, error: `no stage is active — call foundry_run() to get a dispatch, then begin the stage with foundry_stage_begin` };
