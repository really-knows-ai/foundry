...
export function guardSetupInconsistent(lastResult) {
  if (lastResult) return violation('WORK.md needs setup (no stages configured), but you passed lastResult — stages are configured during setup. Call foundry_cycle_run() without any arguments to initialise the cycle first', ['WORK.md']);
  return null;
}
...
export function guardOrphanedStage(activeStage, lastResult) {
  if (activeStage && !lastResult) {
    return violation(
      `stage "${activeStage.stage}" is active but you called foundry_cycle_run() without lastResult — ` +
      ...
      `call foundry_stage_end to close it, then foundry_cycle_run() to get the next action. ` +
...
export function guardMissingLastStage(lastStage) {
  if (!lastStage) return violation(
    ...
    'Call foundry_cycle_run() without arguments to sort and get the next action.',
  );
  return null;
}
