...
function resolveSingleCycle(startCycles, explicitCycle) {
  if (startCycles.length === 0) return { error: 'flow has no start cycle' };
  if (explicitCycle) {
    if (!startCycles.includes(explicitCycle)) {
      return { error: 'cycle ' + explicitCycle + ' is not a start cycle of this flow' };
    }
    return { cycle: explicitCycle };
  }
  if (startCycles.length > 1) {
    return { error: 'flow has multiple start cycles (' + startCycles.join(', ') + '); pass cycle to choose one' };
  }
