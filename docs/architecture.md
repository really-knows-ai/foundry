...
- **`requireOnConfigBranch`** — accepts only `config/<description>`. Used by schema-mutation tools (`foundry_config_create_*`, `foundry_memory_create_*`, `foundry_memory_create_extractor`, memory admin tools). Dry-run is rejected by design — schema must change on a real config branch.
...
- **Blocked tools.** All mutation tools refuse to run and return an error referencing the failure reason:
  ...
  - **Memory admin:** `foundry_memory_init`, `foundry_memory_reset`, `foundry_memory_vacuum`, `foundry_memory_reembed`, `foundry_memory_create_entity_type`, `foundry_memory_create_edge_type`, `foundry_memory_rename_entity_type`, `foundry_memory_rename_edge_type`, `foundry_memory_drop_entity_type`, `foundry_memory_drop_edge_type`, `foundry_memory_create_extractor` (read-only `foundry_memory_validate` and `foundry_memory_dump` remain callable)
