...
**How to navigate:** Start with the **Start here** section to build conceptual foundations and practical operating knowledge before doing any work. Move into **Reference** when you need detailed specifications, tool documentation, or must understand design principles and safety guarantees for implementation or tooling work. Reach for **Contributors** if you are extending Foundry's internals (memory subsystem, token lifecycle, mutation enforcement) or maintaining core systems. Each document is self-contained and cross-linked; follow references as you need them.
...
**Reading order:** Work through them in order; getting-started.md builds muscle memory, and concepts.md provides reference depth. Most implementers spend 1–2 hours on getting-started before moving to Reference materials.
...
- **getting-started.md** — Complete end-to-end installation, bootstrap (`init-foundry`), and first flow walkthrough. Read this immediately after installing the plugin and before authoring any of your own configuration. 
  ...
  Implementers must follow every step and complete the bootstrap; architects typically skim for structure before moving to concepts.md and architecture.md to reason about their designs.
...
- **concepts.md** — The glossary and conceptual foundation for Foundry's key terms and ideas, arranged top-down from flows through cycles, stages, artefacts, and feedback loops. 
  ...
  Reach for this when you encounter terminology you need to understand, or as a reference map before diving into work-spec.md or architecture.md. Defines each concept affirmatively with concrete examples and links outward to spec documents that elaborate them in detail. 
...
- **tools.md** — Categorical index and reference documentation for all 64 custom tools, organised by family (lifecycle, artefacts, feedback, config, memory, etc.) with complete signatures and permissions. 
...
- **memory-maintenance.md** — Cozo 0.7 adaptations, memory session lifecycle notes, and derived implementation wisdom for contributors working on the memory subsystem. 
  ...
  Captures learning from actual bugs and debugging sessions so the next maintainer does not have to discover them again through painful iteration. Core contributors should read this before touching memory code; it explains why certain patterns are necessary, which changes cascade across the system, and common pitfalls to avoid.
