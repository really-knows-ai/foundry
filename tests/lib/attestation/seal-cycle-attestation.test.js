...
describe('Group A1 — all lines have hash mismatches', () => {
  it('seals with composite_status mixed when all stage lines have hash mismatches and are included with _hash_mismatch flag', async () => {
    ...
    assert.ok(sealed.stage_attestations.every(a => a._hash_mismatch === true), 'all tampered attestations carry _hash_mismatch flag');
...
describe('Group E — mixed valid, corrupt, and tampered lines', () => {
  it('builds composite from all parseable lines with warnings for unparseable and hash mismatch', async () => {
    const line1 = makeStageLine(STAGE_1);
    const line3 = makeStageLine(STAGE_3);
    ...
    try {
      ...
      assert.equal(hasTampered, true, 'tampered attestation is included in the composite with _hash_mismatch flag');
      assert.match(sealed._hash, /^[0-9a-f]{64}$/);
